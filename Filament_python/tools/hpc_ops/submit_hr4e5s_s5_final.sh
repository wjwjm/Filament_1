...
set -euo pipefail
readonly REPO="$1" RUN_ROOT="$2" EXPECTED_SHA="$3" PREFLIGHT="$4" CASE_MODE="$5" REFERENCE_CASE_ROOT="$6" REFERENCE_INPUT_MANIFEST="$7" REFERENCE_PREFLIGHT="$8"
...
test -d "$REFERENCE_CASE_ROOT/lifecycle" && test -d "$REFERENCE_CASE_ROOT/optical" && test -f "$REFERENCE_INPUT_MANIFEST" && test -f "$REFERENCE_PREFLIGHT"
case "$CASE_MODE" in
...
PY
"$PYTHON" "$REPO/Filament_python/tools/run_hr4e5s_s5_final.py" validate-reference --reference-root "$REFERENCE_CASE_ROOT" --input "$RUN_ROOT/s5_final_input_manifest.json" --candidate-preflight "$PREFLIGHT" --reference-input "$REFERENCE_INPUT_MANIFEST" --reference-preflight "$REFERENCE_PREFLIGHT" >/dev/null
export_args="ALL,EXPECTED_GIT_SHA=$EXPECTED_SHA,REPO_DIR=$REPO,RUN_ROOT=$RUN_ROOT,CASE_MODE=$CASE_MODE,INPUT_MANIFEST=$RUN_ROOT/s5_final_input_manifest.json,LUT_WORKSPACE=$RUN_ROOT/lut_workspace,REFERENCE_CASE_ROOT=$REFERENCE_CASE_ROOT,REFERENCE_INPUT_MANIFEST=$REFERENCE_INPUT_MANIFEST,REFERENCE_PREFLIGHT=$REFERENCE_PREFLIGHT"
