#!/usr/bin/env bash
# Detach the bounded P4 preflight/launcher chain from an SSH foreground timeout.
set -euo pipefail
...
readonly P4_LAUNCHER="$REPO/Filament_python/tools/hpc_ops/submit_hr4e5p_p4.sh"
...
test -x "$PYTHON" && test -f "$PREFLIGHT_RUNNER" && test -f "$P4_LAUNCHER"
...
with open(temporary,'w',encoding='utf-8',newline='\n') as handle:
    json.dump({'schema':'khz_filament.hr4e5p.p4_async_status.v1','state':state,'stage':stage,'exit_code':int(code)},handle,indent=2,sort_keys=True)
    handle.write('\n')
...
PY
  ...
  bash "$P4_LAUNCHER" "$REPO" "$RUN_ROOT" "$EXPECTED_SHA" "$PREFLIGHT_OUT" "$LAUNCH_MODE" >"$LAUNCHER_STDOUT" 2>"$LAUNCHER_STDERR"
  launcher_rc=$?
...
os.replace(temporary,path)
PY
printf '{"schema":"filament.hpc_ops.write_receipt.v1","ok":true,"state":"completed","operation_state":"started","status":"%s","pid":%s,"run_root":"%s","stage":"P4"}\n' "$STATUS" "$child_pid" "$RUN_ROOT"
