#!/usr/bin/env bash
# S3 no-submit source/provenance/LUT validation.  It creates one new evidence root.
set -euo pipefail
...
test ! -e "$RUN_ROOT" && test ! -e "$OUT" && test ! -e "$ERROR_FILE"
umask 077
...
"$PYTHON" "$REPO/Filament_python/tools/hpc_ops/provenance_v2.py" create --repo "$REPO" --output "${OUT%.json}_provenance_v2.json" --tracked Filament_python/KHz_filament/hr4e5s_s3.py Filament_python/KHz_filament/hr4e5s_streaming.py Filament_python/KHz_filament/hr4.py Filament_python/KHz_filament/hr4c_state.py Filament_python/KHz_filament/propagate.py Filament_python/KHz_filament/thermalization.py Filament_python/KHz_filament/slow_state.py Filament_python/tools/run_hr4e5s_s3.py Filament_python/tools/hr4e5s_s3.sbatch Filament_python/tools/hpc_ops/audit_hr4e5s_s3_lut_workspace.py Filament_python/tools/hpc_ops/run_hr4e5s_s3_preflight.sh Filament_python/tools/hpc_ops/submit_hr4e5s_s3.sh --external "$SOURCE_MANIFEST" "$SOURCE_STATE" "$SOURCE_CONFIG" >/dev/null 2>"$ERROR_FILE"
...
"$PYTHON" "$REPO/Filament_python/tools/hpc_ops/audit_hr4e5s_s3_lut_workspace.py" --config "$SOURCE_CONFIG" --workspace "$LUT_WORKSPACE" --out "$RUN_ROOT/hr4e5s_s3_lut_workspace.json" >"$RUN_ROOT/hr4e5s_s3_lut_workspace.stdout.json" 2>"$RUN_ROOT/hr4e5s_s3_lut_workspace.stderr.txt"
...
"$PYTHON" - "$OUT" "$RUN_ROOT" "$LUT_WORKSPACE" "$EXPECTED_SHA" "$SOURCE_MANIFEST" "$SOURCE_STATE" "$SOURCE_CONFIG" "$RUN_ROOT/hr4e5s_s3_input_manifest.json" "$RUN_ROOT/hr4e5s_s3_lut_workspace.json" <<'PY'
import hashlib,json,sys
out,run_root,lut_workspace,sha,manifest_path,state_path,config_path,input_path,lut_path=sys.argv[1:]
def digest(path):
    h=hashlib.sha256()
...
lut=json.load(open(lut_path,encoding='utf-8'))
...
assert lut['status']=='PASS' and lut['config_sha256']==source['config_sha256']
assert lut['workspace']==lut_workspace and len(lut['lut_records']) >= 2
json.dump({'schema':'khz_filament.hr4e5s.s3.preflight.v2','status':'PASS','git_sha':sha,'run_root':run_root,'source_manifest':manifest_path,'source_manifest_sha256':digest(manifest_path),'source_state':state_path,'source_state_file_sha256':source['hr3b_state_file_sha256'],'source_state_array_sha256':source['hr3b_state_sha256'],'source_config':config_path,'source_config_sha256':source['config_sha256'],'s3_input_manifest':'hr4e5s_s3_input_manifest.json','s3_input_manifest_sha256':digest(input_path),'lut_workspace':'lut_workspace','lut_workspace_sha256':digest(lut_path),'lut_records':lut['lut_records'],'screen_indices':case['screen_indices'],'gpu_matrix':{'batch':1,'streaming_repeat_1':2,'streaming_repeat_2':2}},open(out,'w',encoding='utf-8'),indent=2,sort_keys=True)
PY
