...
LUT_SCIENTIFIC_IDENTITY="$($PYTHON - "$RUN_ROOT/hr4e5s_s5_lut_workspace.json" <<'PY'
...
"$PYTHON" - "$OUT" "$RUN_ROOT" "$EXPECTED_SHA" "$SOURCE_MANIFEST" "$SOURCE_STATE" "$SOURCE_CONFIG" "$RUN_ROOT/hr4e5s_s5_input_manifest.json" "$RUN_ROOT/hr4e5s_s5_lut_workspace.json" "$LUT_SCIENTIFIC_IDENTITY" <<'PY'
import hashlib,json,sys
out,root,sha,manifest,state,config,input_path,lut_path,lut_scientific_identity=sys.argv[1:]
def digest(path):
 h=hashlib.sha256()
