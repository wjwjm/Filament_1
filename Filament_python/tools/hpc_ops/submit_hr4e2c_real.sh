#!/usr/bin/env bash
# Submit the fixed 3-screen x 3-grid E2-C validation-only hydro matrix.
set -euo pipefail
...
readonly -a DX_UM=(20 10 5)
for screen_pos in "${!SCREEN_IDS[@]}"; do
    for dx in "${DX_UM[@]}"; do
        test ! -e "$RUN_ROOT/E2C_${SCREEN_IDS[$screen_pos]}_dx${dx}um_dt0p125us"
    done
done
...
for screen_pos in "${!SCREEN_IDS[@]}"; do
    screen_id="${SCREEN_IDS[$screen_pos]}"
    screen_index="${SCREEN_INDEXES[$screen_pos]}"
    screen_z_m="${SCREEN_Z_M[$screen_pos]}"
