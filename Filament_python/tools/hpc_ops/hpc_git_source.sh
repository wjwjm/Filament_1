...
prepare_stable_bundle() {
    ...
    [[ -n "$BUNDLE" && -n "$BUNDLE_SHA" ]] || return 1
    [[ "$BUNDLE_SHA" =~ ^[0-9a-fA-F]{64}$ ]] || return 1
    [[ -f "$BUNDLE" && ! -L "$BUNDLE" ]] || return 1
    bundle_real="$(realpath -e -- "$BUNDLE" 2>/dev/null)" || return 1
    [[ "$bundle_real" == "$BUNDLE" ]] || return 1
    STABLE_BUNDLE="$(mktemp "$STAGING_ROOT/.verified-bundle.XXXXXX")" || return 1
    chmod 600 -- "$STABLE_BUNDLE" || return 1
    cp -- "$bundle_real" "$STABLE_BUNDLE" || return 1
    actual_sha="$(sha256sum -- "$STABLE_BUNDLE" 2>/dev/null | awk '{print $1}' || true)"
    [[ "${actual_sha,,}" == "${BUNDLE_SHA,,}" ]] || return 1
    VERIFY_ROOT="$(mktemp -d "$STAGING_ROOT/.bundle-verify.XXXXXX")" || return 1
    chmod 700 -- "$VERIFY_ROOT" || return 1
    git -C "$VERIFY_ROOT" init --bare >/dev/null 2>&1 || return 1
    git -C "$VERIFY_ROOT" bundle verify "$STABLE_BUNDLE" >/dev/null 2>&1 || return 1
    heads="$(git bundle list-heads "$STABLE_BUNDLE" 2>/dev/null || true)"
    printf '%s\n' "$heads" | awk -v h="$EXPECTED_HEAD" -v r="$REF" '$1 == h && $2 == r { found=1 } END { exit(found ? 0 : 1) }'
}
...
clone_from_source() {
    ...
    mv -- "$temporary_target" "$TARGET" || return 1
    return 0
}
...
prepare_stable_bundle || fail "verified bundle fallback failed" "$RC_BUNDLE"
SOURCE_CLASS="verified_bundle_non_strict"
if [[ "$MODE" == clone ]]; then
    clone_from_source "$STABLE_BUNDLE" || fail "bundle clone failed" "$RC_GIT"
else
