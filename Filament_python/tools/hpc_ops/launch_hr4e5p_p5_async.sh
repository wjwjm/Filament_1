#!/usr/bin/env bash
# Detach P5 preflight/submission from the SSH foreground timeout.
set -euo pipefail
...
readonly P5_LAUNCHER="$REPO/Filament_python/tools/hpc_ops/submit_hr4e5p_p5.sh"
...
test -x "$PYTHON" && test -f "$PREFLIGHT_RUNNER" && test -f "$P5_LAUNCHER"
...
with open(temporary,'w',encoding='utf-8',newline='\n') as handle:
    json.dump({'schema':'khz_filament.hr4e5p.p5_async_status.v1','state':state,'stage':stage,'exit_code':int(code)},handle,indent=2,sort_keys=True)
    handle.write('\n')
...
PY
  ...
  if [[ "$preflight_rc" != 0 || "$preflight_json_rc" != 0 ]]; then
    failure_rc="$preflight_rc"; if [[ "$failure_rc" == 0 ]]; then failure_rc=1; fi
    write_status FAILED preflight "$failure_rc"; exit 0
  fi
  bash "$P5_LAUNCHER" "$REPO" "$RUN_ROOT" "$EXPECTED_SHA" "$PREFLIGHT_OUT" "$LAUNCH_MODE" >"$LAUNCHER_STDOUT" 2>"$LAUNCHER_STDERR"
  launcher_rc=$?
...
import json,os,sys
path,pid=sys.argv[1:]
record=json.load(open(path,encoding='utf-8')); record['pid']=int(pid)
temporary=path + '.tmp'
with open(temporary,'w',encoding='utf-8',newline='\n') as handle:
    json.dump(record,handle,indent=2,sort_keys=True); handle.write('\n')
os.replace(temporary,path)
PY
printf '{"schema":"filament.hpc_ops.write_receipt.v1","ok":true,"state":"completed","operation_state":"started","status":"%s","pid":%s,"run_root":"%s","stage":"P5"}\n' "$STATUS" "$child_pid" "$RUN_ROOT"
