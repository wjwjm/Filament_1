...
from KHz_filament.hr4e5p_p5 import finalize_p5, write_input_manifest as write_p5_input_manifest
...
def command_p5_manifest(args: argparse.Namespace) -> int:
    write_p5_input_manifest(args.input, args.out_json, args.out_csv)
    return 0


def command_p5_finalize(args: argparse.Namespace) -> int:
    finalize_p5(args.root, args.manifest, args.receipt)
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    serial = sub.add_parser("serial"); serial.add_argument("--state", type=Path, required=True); serial.add_argument("--out", type=Path, required=True); serial.add_argument("--batch-intervals", type=int, default=1); serial.set_defaults(func=command_serial)
    worker = sub.add_parser("worker"); worker.add_argument("--partition", type=Path, required=True); worker.add_argument("--worker-index", type=int); worker.add_argument("--out-dir", type=Path, required=True); worker.set_defaults(func=command_worker)
    gather = sub.add_parser("gather"); gather.add_argument("--partition", type=Path, required=True); gather.add_argument("--worker-dir", type=Path, required=True); gather.add_argument("--out", type=Path, required=True); gather.add_argument("--batch-intervals", type=int, default=1); gather.set_defaults(func=command_gather)
    compare = sub.add_parser("compare"); compare.add_argument("--input", type=Path, required=True); compare.add_argument("--serial", type=Path, required=True); compare.add_argument("--gather", type=Path, required=True); compare.add_argument("--out", type=Path, required=True); compare.set_defaults(func=command_compare)
    ...
    p5_manifest = sub.add_parser("p5-manifest"); p5_manifest.add_argument("--input", type=Path, required=True); p5_manifest.add_argument("--out-json", type=Path, required=True); p5_manifest.add_argument("--out-csv", type=Path, required=True); p5_manifest.set_defaults(func=command_p5_manifest)
    p5_finalize = sub.add_parser("p5-finalize"); p5_finalize.add_argument("--root", type=Path, required=True); p5_finalize.add_argument("--manifest", type=Path, required=True); p5_finalize.add_argument("--receipt", type=Path, required=True); p5_finalize.set_defaults(func=command_p5_finalize)
